...
def create_workload():
    ...
    # SENIOR LOGIC: Qualification Check (Domain Protection)
...
def check_conflict(day, timeslot, teacher_id=None, room_id=None, section_id=None):
    ...
    if teacher_id:
        if query.filter_by(teacher_id=teacher_id).first():
            return "Teacher already has a class in this slot."
    
    if room_id:
        if query.filter_by(room_id=room_id).first():
            return "Room is already occupied in this slot."
            
    if section_id:
        if query.filter_by(section_id=section_id).first():
            return "Section already has a class in this slot."
...
def generate_timetable():
    ...
    1. Hierarchy: Dept -> Program -> Batch -> Section
    2. Domain Protection: Only qualified teachers.
    3. Teacher Availability: Respects teacher.availability JSON.
    4. Room Matching: Correct Room Type (Theory/Lab) and Capacity.
    5. Soft Constraint: Compactness (fills slots early to avoid gaps).
    """
    data = request.json
    ...
    # 1. Clear existing entries for this department
    ...
    # Helper for teacher availability check
    def is_teacher_available(teacher, day, slot):
        if not teacher.availability:
            return True # Assume always available if not specified
        # Assuming availability is a dict of day -> list of free slots
        day_availability = teacher.availability.get(day, [])
        return slot in day_availability

    # 3. Hierarchy Traversal
    for program in dept.programs:
        for batch in program.batches:
            for section in batch.sections:
                # Get workloads for this section
                workloads = Workload.query.filter_by(section_id=section.id).all()
                
                # Soft Constraint: Prioritize days/slots to keep schedule compact
                for workload in workloads:
                    ...
                    found_slot = False
                    # Optimization: Iterate day by day, slot by slot (First-Fit)
                    for day in days:
                        ...
                        for slot in timeslots:
                            ...
                            # A. Domain qualification check
                            if course not in teacher.qualified_courses:
                                errors.append(f"Teacher {teacher.name} not qualified for {course.name}")
                                break 
                            ...
                            # C. Find a suitable Room
                            # Criteria: Matching Type (Theory/Lab) and Capacity >= Students
                            potential_rooms = Room.query.filter(
                            ...
                            for room in potential_rooms:
                                # D. Conflict Check (Overlap)
                                conflict = check_conflict(day, slot, 
                    ...
                    if allocated_hours < workload.hours_per_week:
                        errors.append(f"Could only allocate {allocated_hours}/{workload.hours_per_week} hours for {course.name} (Section {section.name})")
